...
@synthesize provider,providerTimer;
...
{
    providerConfiguration = [[CXProviderConfiguration alloc] initWithLocalizedName:@"Incoming VoIP demo call"];
    providerConfiguration.supportsVideo = NO;
    providerConfiguration.maximumCallGroups = 1;
    //providerConfiguration.iconTemplateImageData = UIImagePNGRepresentation([UIImage imageNamed:@"img.png"]);
    //providerConfiguration.ringtoneSound = @"ring.mp3";
...
-(void)displayIncomingCall:(NSUUID *)uuid handle:(NSString *)handle hasVideo:(BOOL)flag withCompletion:(void(^)(NSError *error))completion
{
    ...
    update.remoteHandle = [[CXHandle alloc] initWithType:CXHandleTypePhoneNumber value:@"Mahadev iPad"];
...
- (void)provider:(CXProvider *)provider performEndCallAction:(CXEndCallAction *)action
{
    ...
    [action fulfillWithDateEnded:[NSDate new]];
